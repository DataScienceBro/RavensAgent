...
# 3X3 RULE FUNCTIONS (use graphical location/context)
# indentical3 = lambda imgA, imgB, imgC, threshold: np.count_nonzero(imgA - imgB) < threshold and np.count_nonzero(imgA - imgC) < threshold and np.count_nonzero(imgC - imgB) < threshold

# quantity based addition
addQuantity3 = lambda imgA, imgB, imgC, imgSize: np.count_nonzero(imgA == 0) + np.count_nonzero(imgB == 0) - np.count_nonzero(imgC == 0) < imgSize * 0.08
